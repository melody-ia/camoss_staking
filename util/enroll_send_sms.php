...
$sms_contents = "[".CONFIG_TITLE."] 인증번호 (".$otp_key.") 를 입력해주세요.";
...
$insert_auth_sql = "INSERT INTO `auth_sms`(mb_id,auth_key,phone_number,auth_start_date,auth_check) VALUES('{$mb_id}','{$opt_key_secure}','{$receive_number}','{$datetime}',0)";

if($debug){
    print_R($insert_auth_sql);
}else{
    sql_query($insert_auth_sql);
}
