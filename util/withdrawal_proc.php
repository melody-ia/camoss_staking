...
$close_withdraw = strtotime($withdrawal_end_time);

if ($use_withdraw_time && ($current_time <= $open_withdraw || $current_time >= $close_withdraw)) {
	echo (json_encode(array("result" => "Failed", "code" => "0002","sql"=>"출금시간은 평일 {$withdrawal_start_time}:00 ~ {$withdrawal_end_time}:00 까지 이용 가능합니다.")));
	return false;
}
