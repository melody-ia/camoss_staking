...
if ($is_member)
{
    $sql_common = " from g5_order where mb_id = '{$member['mb_id']}' ";
}
else if ($od_id && $od_pwd) // 비회원인 경우 주문서번호와 비밀번호가 넘어왔다면
{
    if( defined('G5_MYSQL_PASSWORD_LENGTH') && strlen($od_pwd) === G5_MYSQL_PASSWORD_LENGTH ) {
        $sql_common = " from g5_order where od_id = '$od_id' and od_pwd = '$od_pwd' ";
    } else {
        $sql_common = " from g5_order where od_id = '$od_id' ";
